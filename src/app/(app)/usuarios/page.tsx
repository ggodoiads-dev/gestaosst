...
export default async function UsuariosPage() {
  ...
  const [users, roles, units, areas] = await Promise.all([
    listUsers(),
    listRoles(),
    listUnits(),
    listAreas(),
      ...
      <PageBody>
        <Card>
          <CardHeader>
            ...
            <CreateUserDialog roles={roles} units={units} areas={areas} />
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {users.length === 0 && <TableEmpty colSpan={7} />}
                {users.map((u) => (
                  <TableRow key={u.id} className={!u.active ? "opacity-60" : undefined}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <EditUserDialog user={u} roles={roles} units={units} areas={areas} />
