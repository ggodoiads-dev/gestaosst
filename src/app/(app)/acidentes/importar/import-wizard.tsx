...
import { ACCIDENT_IMPORT_FIELDS, type AccidentImportMapping } from "@/domain/accident/import-fields";
import {
  uploadAccidentSpreadsheetAction,
  previewAccidentImportAction,
  commitAccidentImportAction,
  type UploadResult,
} from "@/server/actions/accident-import.actions";
import type { AccidentImportRowResult } from "@/server/services/accident-import.service";
...
export function ImportWizard() {
  ...
  const [mapping, setMapping] = useState<AccidentImportMapping>({});
  const [previewRows, setPreviewRows] = useState<AccidentImportRowResult[]>([]);
  ...
  function handleUpload() {
    ...
    startTransition(async () => {
      const res: UploadResult = await uploadAccidentSpreadsheetAction(formData);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  function handleBuildPreview() {
    setError(null);
    startTransition(async () => {
      const res = await previewAccidentImportAction(rows, mapping);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  function handleCommit() {
    setError(null);
    startTransition(async () => {
      const res = await commitAccidentImportAction(previewRows);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  const toCreate = previewRows.filter((r) => r.action === "create").length;
  const withErrors = previewRows.filter((r) => r.action === "error").length;
  const withUnmatchedCollaborators = previewRows.filter((r) => r.unmatchedNames.length > 0).length;
      ...
      {step === "mapping" && (
        <>
          <CardHeader>
            ...
            <CardDescription>
              Diga qual coluna da planilha corresponde a cada campo. Data, Tipo, Severidade e Descrição são
              obrigatórios. Área e Colaboradores envolvidos precisam já estar cadastrados (casados pelo nome).
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {ACCIDENT_IMPORT_FIELDS.map((field) => (
      ...
      {step === "review" && (
        <>
          <CardHeader>
            ...
            <CardDescription>
              {toCreate} registro(s) pronto(s) pra importar{withErrors > 0 ? `, ${withErrors} com erro` : ""}
              {withUnmatchedCollaborators > 0
                ? `. ${withUnmatchedCollaborators} linha(s) com colaborador não encontrado (o acidente é criado assim mesmo, só sem esse envolvido).`
                : "."}
            </CardDescription>
          </CardHeader>
              ...
              <TableBody>
                {previewRows.map((row) => (
                  <TableRow key={row.rowIndex}>
                    ...
                    <TableCell>{row.dateParsed ? formatDate(row.dateParsed) : row.date || "—"}</TableCell>
                    ...
                    <TableCell className="text-foreground-subtle">
                      ...
                      {row.unmatchedNames.length > 0 && (
                        <Badge tone="warning" title={row.unmatchedNames.join(", ")} className="ml-1.5">
                          {row.unmatchedNames.length} não encontrado(s)
                        </Badge>
                      )}
                      {row.involvedCollaboratorIds.length === 0 && row.unmatchedNames.length === 0 && "—"}
                    </TableCell>
                    <TableCell>
          ...
          <CardFooter className="justify-between">
            ...
            <Button onClick={handleCommit} loading={pending} disabled={toCreate === 0}>
      ...
      {step === "done" && result && (
        <>
          ...
          <CardContent className="flex flex-col gap-2 text-sm text-foreground-subtle">
            <p>{result.created} acidente(s) importado(s).</p>
            {result.errors > 0 && (
