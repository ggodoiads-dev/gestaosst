...
import { EQUIPMENT_IMPORT_FIELDS, type EquipmentImportMapping } from "@/domain/equipment/import-fields";
import {
  uploadEquipmentSpreadsheetAction,
  previewEquipmentImportAction,
  commitEquipmentImportAction,
  type UploadResult,
} from "@/server/actions/equipment-import.actions";
import type { EquipmentImportRowResult } from "@/server/services/equipment-import.service";
...
export function ImportWizard() {
  ...
  const [mapping, setMapping] = useState<EquipmentImportMapping>({});
  const [previewRows, setPreviewRows] = useState<EquipmentImportRowResult[]>([]);
  ...
  function handleUpload() {
    ...
    startTransition(async () => {
      const res: UploadResult = await uploadEquipmentSpreadsheetAction(formData);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  function handleBuildPreview() {
    setError(null);
    startTransition(async () => {
      const res = await previewEquipmentImportAction(rows, mapping);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  function handleCommit() {
    setError(null);
    startTransition(async () => {
      const res = await commitEquipmentImportAction(previewRows);
      if (!res.ok) {
        setError(res.error);
        return;
      }
        ...
        <>
          ...
          <CardContent className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {EQUIPMENT_IMPORT_FIELDS.map((field) => (
      ...
      {step === "done" && result && (
        <>
          ...
          <CardContent className="flex flex-col gap-2 text-sm text-foreground-subtle">
            <p>{result.created} equipamento(s) criado(s).</p>
            <p>{result.updated} equipamento(s) atualizado(s).</p>
            {result.errors > 0 && (
