...
export async function GET(request: NextRequest) {
  ...
  const restoreEmpilhadeira = request.nextUrl.searchParams.get("restoreEmpilhadeira") === "1";

  if (restoreEmpilhadeira) {
    const updated = await db.jobFunction.updateMany({
      where: { name: { contains: "empilhadeira", mode: "insensitive" } },
      data: { requiresNr: true },
    });
    return NextResponse.json({ ok: true, restored: updated.count });
  }
