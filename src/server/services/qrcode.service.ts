...
/** Gera o QR Code (data URL) apontando para a ficha do colaborador, mesma rota `/q/[token]`. */
export async function generateCollaboratorQrCode(qrToken: string, baseUrl: string): Promise<string> {
  const url = `${baseUrl}/q/${qrToken}`;
  return QRCode.toDataURL(url, { margin: 1, width: 240, color: { dark: "#1a1d23", light: "#ffffff" } });
}
